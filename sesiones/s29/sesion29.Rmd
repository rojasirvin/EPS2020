...
$$\hat{Y}_{N+1}=\hat{\alpha}+X_{T+1}'\hat{\beta}$$
- Planteamos otra función de pérdida

$$(Y_{N+1}-\hat{Y}_{N+1})^2$$
- Lo cual no produce el estimador de MCO
...
- Escacez: aunque hay muchas características, pocas $X_i$ tienen poder predictivo
...
- Optimizar para todos los posibles $k$ y $c$ de tal modo que se minimice $Q(k,c)$
...
- En vez de predecir $Y_i$ usando un árbol, predecimos usando el promedio obtenido por el bosque (*bagging*)
