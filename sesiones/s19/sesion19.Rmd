...
- Ejemplo: areas de mercados desginados (DMA) que abarcan condados en más de un estado en EUA
...
# Efectos de tratamiento espaciales
...
- Calcular para cada punto en la muestra calcular la distancia a los puntos $b$ de la frontera: $f_b(S_i)$
...
- Agregamos: $\hat{\tau}=(\sum_{g=1}^{G}\hat{\tau}(b^g)\hat{f}(b^g))/(\sum_{g=1}^{G}\hat{f}(b^g))$
